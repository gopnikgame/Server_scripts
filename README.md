...
> **Требуется тестирование на живой системе.** Локальные тесты проверяют
> синтаксис, валидацию IP/портов, порядок backup-before-reset и SSH drop-in.
> Реальное применение UFW/SSH следует проверять на тестовом VPS с консолью.
...
Проверено 2026-08-14 на одном тестовом KVM VPS с Ubuntu 24.04.4 Noble,
x86-64-v2 и BIOS/GRUB: установка XanMod LTS 6.18, две перезагрузки, сохранение
предыдущего Ubuntu-ядра, применение и постоянство BBR + `fq` прошли успешно.
Реальная VLESS-нагрузка, Secure Boot, UEFI и DKMS на этом стенде не проверены.
